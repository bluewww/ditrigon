#!/usr/bin/env python3
"""
Regression test for DCC SEND listener peer validation.

Trigger setup:
- queue a normal active DCC SEND to nick "victim"
- register victim with a known (different) IPv4 host in channel userlist
- capture the advertised listening port from outbound CTCP
- connect to that port from an unrelated local client

Expected secure behavior:
- unrelated peer must not receive file bytes
"""
                ...
                while b"\r\n" in pending:
                    ...
                    if not sent_welcome and line.startswith(b"USER "):
                        ...
                        if not _safe_send(conn, (":%s!self@127.0.0.1 JOIN :#race" % nick).encode()):
                            return
                        if not _safe_send(conn, b":victim!id@203.0.113.10 JOIN :#race"):
                            return
                        sent_welcome = True
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: dcc_send_listener_hijack_security_poc.py <ditrigon_binary>", file=sys.stderr)
        return 2
    ...
    cfgdir = tempfile.mkdtemp(prefix="hexchat-dcc-send-listener-hijack-")
    try:
        payload_path = os.path.join(cfgdir, "payload.bin")
        with open(payload_path, "wb") as payload:
            payload.write(b"exfiltration-proof\n")
    ...
    if attacker is None:
        ...
        if attacker.connected and len(attacker.received) > 0:
            print("unauthorized peer received file bytes")
            bad = True
